...
local luac_exec = os.getenv("luac")
...
local function scanfile(filename)
    ...
    if not os.execute(luac_exec .. " -o lualint.tmp "..filename) then
    ...
    local bc = assert(io.popen(luac_exec .. " -l -p "..filename))
    ...
    for line in bc:lines() do
        ...
        local found, _, tp, fname = string.find(line, "(%w+) <([^>]+)>")
        if found then
            context = tp
...
local function lint(filename, cnf)
    ...
    for f,t in pairs(refs) do
        for _,r in ipairs(t.SETGLOBAL) do
            if r.name ~= self_name and not declaredset[r.name] then
                was_set[r.name] = true
                if f ~= '*MAIN*' then
                    lint_output(filename, r.lineno, string.format("*** global SET of %s in [%s]", r.name, f) )
                    found_sets = true
                end
            end
        end
    ...
    for f,t in pairs(refs) do
        for _,r in ipairs(t.GETGLOBAL) do
            if cnf.GETGLOBAL and will_warn_for(r.name) then
                lint_output(filename, r.lineno, string.format("*** global GET of %s in [%s]", r.name, f))
                found_gets = true
            end
        end
    end
...
-- by default warn about SETGLOBAL but not GETGLOBAL
local cnf = { GETGLOBAL=true, SETGLOBAL=true }
