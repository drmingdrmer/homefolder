...
fun! xpsnip#parser#CompileLines(lines) abort "{{{
    ...
    for line in lines
        ...
        if status == 'none'
            ...
            if line[ 0 : 4 ] == 'snip '
                let snipname = line[ 5: ]
                let status = 'set_or_body'
            elseif line[ 0 : 5 ] == 'fuzzy '
                " TODO fuzzy snippet
            else
                " TODO invalid 
            endif
        ...
        if status == 'set'
            ...
            " TODO no leading 'set' is an error
            " TODO set
            continue
        endif
...
fun! xpsnip#parser#CreateSnippet(name, filetype, priority, setting, bodylines) "{{{
    let snip_elts = xpsnip#snip#Compile(bodylines)
    let snip = xpsnip#snip#New(name, filetype, priority, setting, bodylines)
